...
# --- Configuration ---
TOKEN = '8013802329:AAHpiTtWYO00c-EBTZhaUoHDpQmp6_q4vFU' # Replace with your actual token
OWNER_ID = '6716407197'# Replace with your Owner ID (primary owner)
OWNER_IDS = {'6716407197'} # Set of owner IDs with full access
ADMIN_ID = '6716407197' # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = 'Goodboy07000' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/+ReBObZDI5HlmNDJk' # Replace with your update channel link
...
admin_ids = OWNER_IDS | {ADMIN_ID} # Set of admin IDs (includes all owners)
bot_locked = False
...
def get_user_file_limit(user_id):
    ...
    if user_id in OWNER_IDS: return OWNER_LIMIT
...
def _logic_send_welcome(message):
    ...
    if bot_locked and user_id not in admin_ids:
        bot.send_message(chat_id, "⚠️ Bot locked by admin. Try later, Thank you  🙋.")
        return
    ...
    expiry_info = ""
    if user_id in OWNER_IDS: user_status = "👑 Owner"
...
def _logic_upload_file(message):
    user_id = message.from_user.id
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "⚠️ Bot locked by admin, cannot accept files. Thank you 🙋{}")
        return
...
def handle_file_upload_doc(message): # Renamed
    ...
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "⚠️ Bot locked, cannot accept files. Thank you 🙋")
        return
...
def process_add_admin_id(message):
    ...
    try:
        ...
        if new_admin_id in OWNER_IDS: bot.reply_to(message, "⚠️ Owner is already Owner."); return
...
def process_remove_admin_id(message):
    ...
    try:
        ...
        if admin_id_remove in OWNER_IDS: bot.reply_to(message, "⚠️ Owner cannot remove self."); return
...
def list_admins_callback(call):
    bot.answer_callback_query(call.id)
    try:
        admin_list_str = "\n".join(f"- `{aid}` {'(Owner)' if aid in OWNER_IDS else ''}" for aid in sorted(list(admin_ids)))
